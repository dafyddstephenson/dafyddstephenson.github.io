...
def open_mdsparallel(VAR,ITERS,CHUNKS,*args,**kwargs):
    GPT=gptdct[VAR]
    DIM=dimdct[VAR]
    fnames=[(VAR+'.'+str(i).zfill(10)+'.data') for i in ITERS]
    open_parallel=[dask.delayed(mds_to_xda)(VAR,f,GPT,DIM,*args,**kwargs) for f in fnames]
    datasetlist=dask.compute(open_parallel)
    DS=xr.concat(datasetlist[0],dim='time').chunk(CHUNKS)
    return DS
################################################################################
